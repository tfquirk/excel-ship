...
func CountShipmentReferences(expectedCounts map[string]map[string]int, file *excelize.File, IPINames map[string]bool) (map[string]map[string]int, map[string]map[string]int) {
	...
	duplicateExpectedCounts := expectedCounts
	// missingIPINames := make(map[string]bool)
			...
			if accountsPayable == "A/P" {
				if IPINames[company] {
					...
					if len(clientID) == 12 {
						...
						delete(duplicateExpectedCounts, fileNumber)
					}
				}
	...
	fmt.Printf("COUNTED: %v file numbers.\n\n", len(countsOfShipmentRefIds))

	return countsOfShipmentRefIds, duplicateExpectedCounts
}
